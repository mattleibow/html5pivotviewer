...
PivotViewer.Views.TableView = PivotViewer.Views.IPivotViewerView.subClass({
    ...
    CreateTableInner: function ( currentFilter, selectedFacet, sortKey, sortReverse ) {
        ...
        if (sortReverse) {
            sortImage = "pv-tableview-sort-up";
            //offset = +40;
        } else {
            sortImage = "pv-tableview-sort-down";
            //offset = -40;
        }
        ...
        if (sortKey == 'pv-key')
            tableContent += " <div style='position:relative;top:" + offset + "' class='" + sortImage + "'></div>";
                           ...
                           for (l = 0; l < this.tiles[j].facetItem.Facets[k].FacetValues.length; l++) {
                              ...
                              if (this.IsFilterVisible (attribute)) {
                                  ...
                                  if (this.tiles[j].facetItem.Href) {
                                      // Value is uri
                                      if (this.IsUri(value))
                                          tableRows.push({key: sortKeyValue, value: "<tr class='pv-tableview-" + oddOrEven +"'><td id='pv-key' class='tooltipinter' title='Toggle item selection' style='color:#009933;cursor:pointer'>" + entity + " " + "<a href=" + this.tiles[j].facetItem.Href.replace(/'/g, "%27") + " target=\"_blank\"><div style='cursor:default;' id='pv-linkout' title='Follow the link'></div></a></td><td id='pv-facet' class='tooltipcustom'  title='Show only this relation' style='color:#009933;cursor:pointer'>" + attribute + "</td><td id='pv-value' class='tooltipinter' title='Filter on this value' style='color:#009933;cursor:pointer'>" + value + " " + "<a href=" + value + " target=\"_blank\"><div style='cursor:default;' id='pv-linkout' title='Follow the link'></div></a></td></tr>"});
                                      else
